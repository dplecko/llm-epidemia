...
df["age"] = (
    df["AGE3"]
    .map(age_map)
    .astype(pd.CategoricalDtype(categories=list(age_map.values()), ordered=True))
)
df["sex"] = (
    df["irsex"]
    .map(lambda x: "male" if x == "1 - Male" else "female")
    .astype("category")
)
...
df["edu"] = (
    ...
    .astype(
        pd.CategoricalDtype(
            categories=list(dict.fromkeys(edu_map.values())), ordered=True
        )
    )
)
...
df["income"] = (
    df["IRPINC3"]
    .map(income_map)
    .astype(pd.CategoricalDtype(categories=list(income_map.values()), ordered=True))
)

# ever variables
df["alc_ever"] = (
    df["alcever"].map(lambda x: "yes" if x == "1 - Yes" else "no").astype("category")
)
df["cig_ever"] = (
    df["cigever"].map(lambda x: "yes" if x == "1 - Yes" else "no").astype("category")
)
df["mj_ever"] = (
    df["mjever"].map(lambda x: "yes" if x == "1 - Yes" else "no").astype("category")
)
df["coc_ever"] = (
    df["cocever"].map(lambda x: "yes" if x == "1 - Yes" else "no").astype("category")
)
df["her_ever"] = (
    df["herever"].map(lambda x: "yes" if x == "1 - Yes" else "no").astype("category")
)

# monthly variables
df["alc_monthly"] = (
    df["alcrec"]
    .map(lambda x: "yes" if x == "1 - Within the past 30 days" else "no")
    .astype("category")
)
df["cig_monthly"] = (
    df["cigrec"]
...
Z = ["age", "sex"]
W = ["edu", "income"]
Y = ["alc_monthly", "cig_monthly", "mj_monthly", "coc_ever"]
