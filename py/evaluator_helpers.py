...
def extract_pv(prompt, levels, mode, model_name, model, second_prompt=None, n_mc=128):
    """
    ...
    if levels is not None and model.get_type() != "API":
        max_tokens = max_lvl_len(levels, model.tokenizer)

    if mode == "logits" and max_tokens == 1:
        return model.lvl_probs(prompt, levels)

    elif (mode == "sample" and levels is not None) or (mode == "logits" and max_tokens > 1):
        return model.lvl_sample(prompt, levels, n_mc, max_batch_size)
    
    elif mode == "sample" and levels is None:
        return model.cts_sample(prompt, n_mc, max_batch_size)

    elif mode == "story":
        if second_prompt is None:
            raise ValueError("second_prompt is required for 'story' mode")
        return model.story_sample(prompt, second_prompt, levels, n_mc, max_batch_size)
