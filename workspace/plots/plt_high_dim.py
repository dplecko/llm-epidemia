...
eval_hdq, _ = build_eval_df(models, task_specs_hd)
eval_hdp, _ = build_eval_df(models, task_specs_hd, prob = True)

eval_hd = pd.concat([eval_hdq, eval_hdp])
...
df_bydim = eval_hd.groupby(["model", "dim", "prob"]).agg(
    score=("score", "mean"),
    # sd_cor=("correlation", "std")
).reset_index()
...
df_bydim["Prompting"] = df_bydata["prob"].map(
...
plt_bydim = (ggplot(df_bydim, aes(x="dim", y="score", fill = "Model")) +
       ...
       theme_bw() + coord_cartesian(ylim=(0, 100)) +
       theme(panel_background=element_rect(fill="white"), plot_background=element_rect(fill="white"),
             legend_position="inside", legend_position_inside=(0.5, 0.9),
             legend_background=element_rect(color="black", fill="white"),
             legend_direction='horizontal',
             axis_text_x=element_text(rotation=30, hjust=0.8, size=12)) +
       facet_grid("Prompting ~ ."))
