#!/usr/bin/env python3
"""
BRFSS Mistral Fine-tuning Script
Based on the obs branch finetune.py but adapted for BRFSS synthetic data with Mistral model
"""
...
model_name = "/local/eb/dp3144/mistral_7b_instruct"
...
# --- training ---
out = "/local/eb/shreyas/models/mistral_7b_instruct_brfss_finetuned"
...
print("Starting BRFSS Mistral fine-tuning...")
trainer.train()
...
plt.ylabel("loss")
plt.legend(); plt.title("Mistral BRFSS CLM fine-tuning loss"); plt.tight_layout()
plt.savefig(os.path.join(out, "loss_plot.png"), dpi=200)
plt.show()

print("Best BRFSS Mistral model saved to:", best_dir)
