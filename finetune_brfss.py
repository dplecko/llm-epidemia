#!/usr/bin/env python3
"""
BRFSS Llama Fine-tuning Script
Based on the obs branch finetune.py but adapted for BRFSS synthetic data
"""
...
# --- data ---
ds = load_from_disk("/home/shreyas/llm-epidemia/datasets/brfss_synth_gemma")  # expects splits: 'train','validation'
model_name = "/local/eb/dp3144/llama3_8b_instruct"
...
g_train = tok_ds["train"].map(group_texts, batched=True, remove_columns=tok_ds["train"].column_names, cache_file_name=None)
g_eval  = tok_ds["validation"].map(group_texts, batched=True, remove_columns=tok_ds["validation"].column_names, cache_file_name=None)
...
# --- training ---
out = "/local/eb/shreyas/models/llama3_8b_instruct_brfss_finetuned"
steps = 10  # Increased for more frequent evaluation
...
print("Starting BRFSS fine-tuning...")
trainer.train()
...
plt.ylabel("loss")
plt.legend(); plt.title("Llama BRFSS CLM fine-tuning loss"); plt.tight_layout()
plt.savefig(os.path.join(out, "loss_plot.png"), dpi=200)
plt.show()

print("Best BRFSS model saved to:", best_dir)
