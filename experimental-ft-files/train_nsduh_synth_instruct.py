#!/usr/bin/env python3
"""
LoRA Fine-tuning script for NSDUH Synthetic Gemma Dataset (Instruct model)
"""
...
def main():
    try:
        logger.info("="*60)
        logger.info("NSDUH SYNTHETIC GEMMA MODEL TRAINING (Instruct)")
        logger.info("="*60)

        base_dir = Path("/home/shreyas/llm-epidemia")
        cache_dir = base_dir / "huggingface_cache"
        model_dir = base_dir / "models" / "nsduh_synth_gemma_instruct"
        ...
        run_name = f"nsduh-synth-gemma-instruct-{timestamp}"
        ...
        wandb.init(
            ...
            config={
                ...
                "model_path": "/local/eb/dp3144/llama3_8b_instruct",
                "method": "LoRA",
        ...
        model_path = "/local/eb/dp3144/llama3_8b_instruct"
        logger.info(f"🤖 Loading local Llama-3 8B Instruct model from: {model_path}")
